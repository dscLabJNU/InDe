...
/*读取文件，将文件内容分块，存入read_queue*/
static void read_file(sds path) {
	static unsigned char buf[DEFAULT_BLOCK_SIZE];   //论文里segment size=20MB,DEFAULT_BLOCK_SIZE=1MB

	sds filename = sdsdup(path);

	if (jcr.path[sdslen(jcr.path) - 1] == '/') {
		/* the backup path points to a direcory */
		sdsrange(filename, sdslen(jcr.path), -1);    //参数-1有问题，去尾
	} else {
