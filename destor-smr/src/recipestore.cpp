...
//static int64_t unique_chunk;
//static int 64_t total_segment_num;
//static int64_t avg_rw;
//static int32_t bound_dedup_ratio=0.5;
...
void init_recipe_store() {
	...
	FILE *fp;
	if ((fp = fopen(count_fname, "r"))) {       //该文件必须存在？？
		/* Read if exists. */
...
struct backupVersion* create_backup_version(const char *path) {
	struct backupVersion *b = (struct backupVersion *) malloc(sizeof(struct backupVersion));
 ...
 * return the offset
 */
void append_n_chunk_pointers(struct backupVersion* b, struct chunkPointer* cp, int n) {
...
struct chunkPointer* read_next_n_chunk_pointers(struct backupVersion* b, int n,	int *k) {
	...
	int num = (b->number_of_chunks - read_chunk_num) > n ?	n : (b->number_of_chunks - read_chunk_num), i;

	struct chunkPointer *cp = (struct chunkPointer *) malloc(sizeof(struct chunkPointer) * num);
...
struct segmentRecipe* new_segment_recipe() {
	struct segmentRecipe* sr = (struct segmentRecipe*) malloc(sizeof(struct segmentRecipe));
	sr->id = TEMPORARY_ID;
