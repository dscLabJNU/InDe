...
static void* append_thread(void *arg) {
	...
	while (1) {
		...
		write_container(c);  //将container c写入container.pool文件中
...
void init_container_store() {
	...
	if ((fp = fopen(containerfile, "r+"))) {           //该文件必须存在？？？
...
void write_container(struct container* c) {
	...
	VERBOSE("Append phase: Writing container %lld of %d chunks", c->meta.id,c->meta.chunk_num);
	...
	if (destor.simulation_level < SIMULATION_APPEND) {
		...
		while (g_hash_table_iter_next(&iter, &key, &value)) {
			struct metaEntry *me = (struct metaEntry *) value;   //key:fp  value:metaEntry
