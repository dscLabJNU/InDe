...
int rewrite_buffer_push(struct chunk* c) {
	...
	if (c->id != TEMPORARY_ID) {
		...
		tmp_record.cid = c->id;
		GSequenceIter *iter = g_sequence_lookup(rewrite_buffer.container_record_seq, &tmp_record,
				(GCompareDataFunc )g_record_cmp_by_id,
				NULL);
		if (iter == NULL) {
			struct containerRecord* record =(containerRecord*) malloc(sizeof(struct containerRecord));
			record->cid = c->id;
			record->size = c->size;
	...
	if (rewrite_buffer.num >= destor.rewrite_algorithm[1]) {//destor.rewrite_algorithm[1]=1024
...
void start_rewrite_phase() {
    ...
    } else if (destor.rewrite_algorithm[0] == REWRITE_CFL_SELECTIVE_DEDUPLICATION) {
