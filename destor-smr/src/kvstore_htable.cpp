...
static void kv_update(kvpair kv, int64_t id){
	...
	memmove(&value[1], value, (destor.index_value_length - 1) * sizeof(int64_t));
	value[0] = id;
}
...
/*利用index/htable文件中的内容构建index htable :key：fp  value:fp+cid(<=8)*/
...
void kvstore_htable_delete(unsigned char* key, int64_t id){
	...
	for(i=0; i<destor.index_value_length; i++){
		if(value[i] == id){
			...
			assert((i == destor.index_value_length - 1) || value[i+1] == TEMPORARY_ID);
