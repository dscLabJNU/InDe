...
void do_backup(char *path) {
	...
	destor.unique_chunk_num += jcr.unique_chunk_num;
	destor.seg_num += jcr.seg_num;
	//destor.avg_dedup_chunk = (destor.stored_chunk_num * DEDUP_REDUCTION_LIM - destor.rewritten_chunk_num) / (1 - DEDUP_REDUCTION_LIM);
	...
	//printf("number of unique chunks destor: %d\n", destor.unique_chunk_num);
