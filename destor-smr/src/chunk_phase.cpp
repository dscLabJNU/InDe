	...
	while (1) {
		...
		while (1) {
			...
			if (memcmp(zeros, nc->data, chunk_size) == 0) {
				VERBOSE("Chunk phase: %ldth chunk  of %d zero bytes", chunk_num++, chunk_size);
				jcr.zero_chunk_num++;
		...
		if(destor.chunk_algorithm == CHUNK_RABIN ||	destor.chunk_algorithm == CHUNK_NORMALIZED_RABIN)
			windows_reset();
	}
	free(leftbuf);
	free(zeros);
...
/*将chunk从read_queue中取出按照分块算法重新分块，并记录数据块的数量jcr.zero_chunk_num\chunk_number,
分块后再压入chunk_queue*/
