...
void destor_start() {
	...
	if ((fp = fopen(stat_file, "r"))) {
		...
		fread(&destor.unique_chunk_num, 8, 1, fp);
		fread(&destor.seg_num, 8, 1, fp);
		//fread(&destor.avg_dedup_chunk, 8, 1, fp);
		...
		destor.unique_chunk_num = 0;
		destor.seg_num = 0;
		//destor.avg_dedup_chunk;
...
void destor_shutdown() {
	...
	fwrite(&destor.unique_chunk_num, 8, 1, fp);
	fwrite(&destor.seg_num, 8, 1, fp);
	//fwrite(&destor.avg_dedup_chunk, 8, 1, fp);
...
void destor_stat() {
	...
	*/
	...
	printf("the number of unique chunks: %lld\n", destor.unique_chunk_num);
	printf("the number of rsegments: %lld\n", destor.seg_num);
